...
func TestBackupKey_PerBackupNameSalting(t *testing.T) {
	mk := testMasterKey(t, 1)
	k1, err := BackupKey(mk, "backup-2026-01-01")
	if err != nil {
		t.Fatal(err)
	}
	k2, err := BackupKey(mk, "backup-2026-01-02")
	if err != nil {
		t.Fatal(err)
	}
	if bytes.Equal(k1, k2) {
		t.Error("BackupKey produced the same key for two different backup names -- a compromised key would expose every backup, not just one")
	}
	// But deterministic for the same name, so a backup can be re-opened later.
	k1Again, err := BackupKey(mk, "backup-2026-01-01")
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(k1, k1Again) {
		t.Error("BackupKey is not deterministic for the same master key and backup name")
	}
}

func TestSessionSecret_BackupKey_DomainSeparated(t *testing.T) {
	mk := testMasterKey(t, 1)
	session, err := SessionSecret(mk)
	if err != nil {
		t.Fatal(err)
	}
	backup, err := BackupKey(mk, "x")
	if err != nil {
		t.Fatal(err)
	}
	if bytes.Equal(session, backup) {
		t.Error("SessionSecret and BackupKey derived the same bytes -- a leaked session secret would also leak backup keys")
	}
}

func TestBackupSealOpen_RoundTrip(t *testing.T) {
	mk := testMasterKey(t, 1)
	plaintext := []byte("a backup archive's worth of bytes")

	ct, err := BackupSeal(mk, "backup-1", plaintext)
	if err != nil {
		t.Fatal(err)
	}
	got, err := BackupOpen(mk, "backup-1", ct)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := BackupOpen(mk, "backup-2", ct); err == nil {
		t.Error("opening with the wrong backup name succeeded -- per-backup keys aren't actually isolating backups")
	}
	otherMK := testMasterKey(t, 2)
	if _, err := BackupOpen(otherMK, "backup-1", ct); err == nil {
		t.Error("opening with the wrong master key succeeded")
	}

	tampered := append([]byte{}, ct...)
	tampered[len(tampered)-1] ^= 0xFF
	if _, err := BackupOpen(mk, "backup-1", tampered); err == nil {
		t.Error("opening tampered ciphertext succeeded -- GCM authentication isn't catching modification")
	}
}

...
- ~~**v2's `/rgdowntime/xml` default response is ~250x larger than v1's...**~~ **FIXED** (commit `e00ab1a`, "Broaden live parity coverage to /rgdowntime, /vosummary, /miscproject"). `internal/xmlapi/rgdowntime.go`'s `BuildDowntimes` now applies `Filters.PastDays` exactly like v1's `past_days`/`_is_shown` (default 0 → past hidden, `-1`/`all` → unbounded, `N` → last N days), with `internal/paritytest/rgdowntime_test.go` covering it. Live-reconfirmed: default response is a small fraction of the `?downtime_attrs_showpast=all` count, not identical to it. This bullet is stale — left here with the fix noted rather than deleted, so the "new findings only visible live" history stays intact.
